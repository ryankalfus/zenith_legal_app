...
- 02.27.2026: Removed admin appointment list filtering tied to candidate directory snapshots so all appointment rows stay visible in shared admin appointment views.
- 02.27.2026: Added appointment-level recruiter assignment (`Ellen`/`Mason`) across request/create/modify flows with persistent Firestore fields (`recruiterId`, `recruiterName`), surfaced recruiter in admin unattended/upcoming/overdue + candidate overdue/upcoming/pending cards, and included recruiter context in Zenith appointment chat updates.
- 02.27.2026: Synced recruiter dropdown sources to live admin users across mobile (admin appointments create/modify, candidate appointment request, assigned recruiter filters), tightened admin appointment create-form Candidate/Recruiter spacing, styled top contact hyperlinks as black bold underlined links, moved Admin Candidate Detail `Assign Firm` button to the end of `Assigned firms`, and removed extra card nesting around Admin Candidates search input.
- 02.27.2026: Upgraded recruiter pickers to candidate-style preview rows (avatar + phone) with live recruiter sync in Admin Candidate Detail `Assigned Recruiter`, Admin Candidate Filters `Assigned recruiter`, and Admin Appointments recruiter selection modals (create/modify), while keeping `None` option support for assigned-recruiter flows.
- 02.27.2026: Removed Mason-only admin gating from active app auth paths (mobile + admin web) so admin access is role-based for all recruiter/admin accounts, synced role-change writes across UID/email-linked user docs on promotion/demotion, and restored admin inbox swipe-left `Delete` action behavior (soft-hide chat until new message) with unswipe flow preserved.
- 02.27.2026: Updated admin chat delete to clear the full conversation on the admin side globally: deleting now marks the conversation hidden for admins and marks every message in that thread `hiddenForAdmin` (batched), so all admin accounts permanently lose that thread history until a new message reopens it.
- 02.27.2026: Hardened admin-role auto-sync for non-Mason accounts by adding email-based admin fallback checks during login (mobile + admin web), upgrading backend `changeUserRole` resolution to map doc-id targets to real auth UIDs and batch-sync role changes across related duplicate user docs, and extending mobile role-sync fallback matching to include `name + phone` identity.
- 02.27.2026: Fixed non-Mason admin desync root cause by replacing client-side email-only admin fallback with backend role self-heal callable (`syncOwnRoleFromProfile`) that syncs canonical admin role + custom claim before admin auth checks (mobile + web), preventing admin UI from loading without real shared-admin read permissions.
